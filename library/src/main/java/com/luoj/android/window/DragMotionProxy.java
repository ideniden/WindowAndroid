...
public class DragMotionProxy {
	...
	final String TAG="DragMotionProxy";
...
//                             Log.d(TAG,"drag_motion_proxy onTouch down");
                             startX = ev.getRawX();
                             startY = ev.getRawY();
...
//						Log.d(TAG,"drag_motion_proxy onTouch move isMoveing:"+isMoveing);
						if (isMoveing) {
							...
							if (Math.abs(startX - ev.getRawX()) > canMoveOffset || Math.abs(startY - ev.getRawY()) > canMoveOffset) {
								isMoveing = true;
//								Log.d(TAG,"开始拖拽，事件由拖拽布局处理，不下放。");
								return true;
							}
						}
						break;
					case MotionEvent.ACTION_UP:// 弹起
//						Log.d(TAG,"drag_motion_proxy onTouch up");
	...
	public boolean handleInterceptTouchEvent(MotionEvent ev) {
		...
		switch (ev.getAction()) {
		case MotionEvent.ACTION_DOWN:
			Log.d(TAG,"drag_motion_proxy handleInterceptTouchEvent down");
			startX = ev.getRawX();
			startY = ev.getRawY();
			if(null!=DragMotionProxy.this.mOnDragListener)DragMotionProxy.this.mOnDragListener.onActionDown(ev);
			break;
		case MotionEvent.ACTION_MOVE:
			Log.d(TAG,"drag_motion_proxy handleInterceptTouchEvent move");
			if (Math.abs(startX - ev.getRawX()) > canMoveOffset || Math.abs(startY - ev.getRawY()) > canMoveOffset) {
				isMoveing = true;
				Log.d(TAG,"开始拖拽，事件由拖拽布局处理，不下放。");
				return true;
